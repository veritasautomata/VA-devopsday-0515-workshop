...
**1 week ahead.** Send the prerequisites email. List exactly what they need: Docker Desktop installed and running, a terminal, an opencode zen API key from `opencode.ai/zen` (free to sign up, ~$1 of credit lasts the workshop). Tell them to run `docker run hello-world` to confirm Docker works. Tell them the workshop repo URL so they can clone it before walking in.

**1 day ahead.** Send a reminder. Include the quick prereq-check script — they run it, get a pass/fail, paste output if they're stuck.

**Day-of, 30 min before.** Test the demo cold yourself. Run `docker compose down -v && docker compose build agent && docker compose up -d pipelock && docker compose run --rm agent` and time it. Whatever it took for you, add 50% for the room. If your build takes 4 minutes, build the image **before** the workshop starts and tell people to do the same. Don't burn 4 minutes of a 45-minute session on a build.

**Open windows ahead of time.**
- Terminal pre-cd'd to the repo, with `.env` already populated
- Browser tab on the slides
- Browser tab on `opencode.ai/zen` (in case someone needs a key during the session)
- Second terminal window for `docker compose logs -f pipelock | jq` during the attack/defense slide
